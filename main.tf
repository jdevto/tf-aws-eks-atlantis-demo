...
module "eks" {
  ...
  cloudwatch_log_group_force_destroy  = var.cloudwatch_log_group_force_destroy
...
# Shared ALB (used by landing page and other ingresses)
module "shared_alb" {
  ...
  vpc_id             = module.vpc.vpc_id
  ingress_group_name = local.shared_alb_ingress_group_name
...
module "landing_page" {
  ...
  shared_alb_security_group_id  = module.shared_alb[0].security_group_id
  enable_https                  = var.enable_https
  certificate_arn               = var.enable_https ? var.certificate_arn : ""
  ssl_redirect                  = var.enable_https
  domain_name                   = var.domain_name
  alb_arn                       = module.shared_alb[0].alb_arn
  ...
  depends_on = [module.eks, module.shared_alb]
}
